...
class enrol_attributes_plugin extends enrol_plugin {
    ...
    public static function handle_profile_update(\core\event\base $event){
        ...
        foreach ($enrol_instances as $instance){
            ...
            $user_enrolment->courseid = $instance->courseid;
            $user_enrolment->enrol = 'attributes';
            $user_enrolment->lastenrol = true;
            ...
            $params = array_merge([$userid], $arraysql['params']);
                ...
                switch ($instance->customint1){
                    case ENROL_ATTRIBUTES_WHENEXPIREDREMOVE:
                        ...
                        $event = \core\event\user_enrolment_deleted::create([
                            ...
                            'other' => ['enrol' => 'attributes', 'userenrolment' => (array)$user_enrolment]
                        ]);
                        $event->trigger();
                        
                        // Remove user from groups only when completely unenrolled
                        if ($groups = $DB->get_records('groups', ['courseid' => $instance->courseid])){
    ...
    public static function process_enrolments($event = null, $instanceid = null){
        ...
        if ($instanceid){
            ...
            $enrol_attributes_records = $DB->get_records('enrol', [
                    'enrol'  => 'attributes',
                    'status' => 0,
                    'id'     => $instanceid
            ]);
        ...
        foreach ($possible_unenrolments as $id => $user_enrolment){
            ...
            if (!$event && !$instanceid && $nbpossunenrol % 1000 === 0 && isset($_SERVER['argv'][0]) && is_string($_SERVER['argv'][0]) &&
                !str_contains($_SERVER['argv'][0], 'phpunit')){
                mtrace('-', '');
            }

            $unenrol_attributes_record = $DB->get_record('enrol', [
                    'enrol'  => 'attributes',
                    'status' => 0,
            ...
            if (!array_key_exists($user_enrolment->userid, $users)){
                ...
                if ($unenrol_attributes_record->customint1 == ENROL_ATTRIBUTES_WHENEXPIREDREMOVE){
                    ...
                    // Remove user from groups only when completely unenrolled
                    if ($groups = $DB->get_records('groups', ['courseid' => $unenrol_attributes_record->courseid])){
                        foreach ($groups as $group){
        ...
        foreach ($enrol_attributes_records as $enrol_attributes_record){
            $nbpossenrol++;
            if (!$event && !$instanceid && isset($_SERVER['argv'][0]) && is_string($_SERVER['argv'][0]) &&
                !str_contains($_SERVER['argv'][0], 'phpunit')){
                // we only want output if runnning within the scheduled task
                mtrace('+', '');
            }
        ...
        foreach ($arraysyntax['rules'] as $rule){
            ...
            if (isset($rule->rules)){
                ...
                if (isset($rule->comp_op) && $rule->comp_op === 'contains'){
                    ...
                    $params[] = '%' . $rule->value . '%';
                } elseif (isset($rule->comp_op) && $rule->comp_op === 'notcontains'){
    ...
    public function get_newinstance_link($courseid){
        ...
        return new moodle_url('/enrol/attributes/edit.php', ['courseid' => $courseid]);
    }
    ...
    public function get_action_icons(stdClass $instance){
        ...
        if (has_capability('enrol/attributes:config', $context)){
            $editlink = new moodle_url("/enrol/attributes/edit.php", [
                    'courseid' => $instance->courseid,
                    'id'       => $instance->id
            ]);
            $icons[] = $OUTPUT->action_icon($editlink,
                    new pix_icon('i/edit', get_string('edit'), 'core', ['class' => 'icon']));
        }
    ...
    */
    ...
    public function add_course_navigation($instancesnode, stdClass $instance){
        ...
        $context = context_course::instance($instance->courseid);
        if (has_capability('enrol/attributes:config', $context)){
            $managelink = new moodle_url('/enrol/attributes/edit.php', [
                    'courseid' => $instance->courseid,
                    'id'       => $instance->id
            ]);
